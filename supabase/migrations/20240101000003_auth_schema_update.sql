-- ============================================================================
-- Phase 2.1.1: Authentication Schema Update & RLS Setup
-- ============================================================================
-- This migration updates the existing schema for user management with:
...
-- Create indexes
CREATE INDEX IF NOT EXISTS idx_seller_agreements_user 
  ON public.seller_agreements(user_id);

CREATE INDEX IF NOT EXISTS idx_seller_agreements_stripe 
  ON public.seller_agreements(stripe_connect_id) 
...
DROP TRIGGER IF EXISTS update_seller_agreements_updated_at ON public.seller_agreements;
...
RETURNS TRIGGER AS $$
BEGIN
  -- Log the deletion (only if audit_logs table exists)
  IF EXISTS (
    SELECT 1 FROM information_schema.tables 
    WHERE table_schema = 'public' AND table_name = 'audit_logs'
  ) THEN
...
CREATE INDEX IF NOT EXISTS idx_profiles_role_flags_seller 
  ON public.profiles((role_flags->>'is_seller')) 
  WHERE (role_flags->>'is_seller')::boolean = true;

CREATE INDEX IF NOT EXISTS idx_seller_agreements_status 
  ON public.seller_agreements(onboarding_status);
